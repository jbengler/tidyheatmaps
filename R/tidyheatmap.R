...
#'   `columns`.
#' @param values Column in the dataframe containing the values to be color coded
...
#'
#' @param annotation_row,annotation_col Column(s) in the dataframe to use for
#'   `row` and `column` annotation.
#' To use multiple columns for annotation combine then by `c(column1, column2)`.
#' @param gaps_row,gaps_col Column in the dataframe to use for use for `row` and
#'   `column` gaps.
#' @inheritParams pheatmap::pheatmap
#'
#' @param show_selected_row_labels,show_selected_col_labels Only display a
#'   subset of selected labels for `rows` and `columns`.
...
#' * `gtable` a `gtable` object containing the heatmap, can be used for combining
...
tidyheatmap <- function(
  ...
  heatmap_data <-
    wrangle_data(df, {{ rows }}, {{ columns }}, {{ values }},
...
wrangle_data <- function(df, rows, columns, values, annotation_row = NULL,
                         annotation_col = NULL, gaps_row = NULL, gaps_col = NULL) {
  m <-
    df %>%
  ...
  list(m = m, annotation_row = annotation_row, annotation_col = annotation_col,
